...
// 🌍 Global Base URL - will auto-detect based on environment
...
async function getSongs(folder) {
    ...
    try {
        ...
        for (let index = 0; index < as.length; index++) {
            const element = as[index];
            if (element.href.endsWith(".mp3")) {
                let songPath = element.href.split(`songs/${folder}/`)[1];
                songs.push(songPath);
            }
        }
...
const playMusic = (track, pause = false) => {
    currentSong.src = `${BASE_URL}/songs/${currFolder}/${encodeURIComponent(track)}`;
    if (!pause) {
        currentSong.play()
            .then(() => {
                play.src = "/img/pause.svg";
        ...
        for (let index = 0; index < array.length; index++) {
            ...
            if (e.href.includes("/songs/") && !e.href.endsWith("/songs/")) {
                ...
                try {
                    ...
                    if (!a.ok) continue; // Skip if no info.json
                    ...
                    cardContainer.innerHTML += `
                        <div data-folder="${folder}" class="card">
                            ...
                            <img src="/songs/${folder}/cover.jpg" alt="${albumData.title}">
                            <h2>${albumData.title}</h2>
                            <p>${albumData.description}</p>
                        </div>
                    `;
...
async function main() {
    // First load default songs
    await getSongs("june");
    if (songs.length > 0) {
        playMusic(songs[0], true);
    }

    // Then display all albums
    await displayAlbum();
