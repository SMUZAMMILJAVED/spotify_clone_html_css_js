...
// Predefined song lists
const SONG_DATA = {
    "june": ["song1.mp3","song2.mp3","song3.mp3","song4.mp3","song5.mp3","song6.mp3"],
    "july": ["song7.mp3","song8.mp3","song9.mp3","song10.mp3","song11.mp3","song12.mp3"],
    "aug": ["song13.mp3","song14.mp3","song15.mp3"]
};

// Predefined album data
const ALBUM_DATA = {
    "june": { "title": "June Collection", "description": "Summer vibes and fresh beats" },
    "july": { "title": "July Hits", "description": "Mid-summer anthems and chill tracks" },
    "aug": { "title": "August Favorites", "description": "Late summer classics and new discoveries" }
};

async function getSongs(folder) {
    currFolder = folder;
    songs = SONG_DATA[folder] ? [...SONG_DATA[folder]] : [];
    let songUL = document.querySelector(".songList")?.getElementsByTagName("ul")[0];
    ...
    for (const song of songs) {
        ...
        songUL.innerHTML += `
            <li> 
                <img class="invert" src="${BASE_URL}/img/music.svg" alt="">
                <div class="info">
                    <div>${displayName}</div>
                    <div>Anss</div>
                </div>
                <div class="playnow">
                    <span>Play Now</span>
                    <img class="invert" src="${BASE_URL}/img/play.svg" alt="">
                </div>
            </li>`;
    }
...
const playMusic = (track, pause = false) => {
    ...
    if (!pause) {
        ...
        const playBtn = document.getElementById("play");
        if (playBtn) playBtn.src = `${BASE_URL}/img/pause.svg`;
    }
    ...
    for (const [folder, albumData] of Object.entries(ALBUM_DATA)) {
        cardContainer.innerHTML += `
            <div data-folder="${folder}" class="card">
                ...
                <img src="${BASE_URL}/songs/${folder}/cover.jpg" 
                     alt="${albumData.title} Cover"
                     onerror="this.src='${BASE_URL}/img/default-cover.jpg'; this.onerror=null;">
                <h2>${albumData.title}</h2>
                <p>${albumData.description}</p>
            </div>
        `;
...
async function main() {
    try {
        ...
        if (playBtn) {
            playBtn.addEventListener("click", () => {
                if (currentSong.paused) {
                    ...
                    playBtn.src = `${BASE_URL}/img/pause.svg`;
                } else {
                    currentSong.pause();
                    playBtn.src = `${BASE_URL}/img/play.svg`;
                }
            });
        }
        ...
        document.getElementById("previous")?.addEventListener("click", () => {
            ...
            else if (index === 0) playMusic(songs[songs.length - 1]);
        });

        document.getElementById("next")?.addEventListener("click", () => {
            const index = songs.indexOf(currentSong.src.split("/").pop());
            if (index >= 0 && index < songs.length - 1) playMusic(songs[index + 1]);
            else if (index === songs.length - 1) playMusic(songs[0]);
        });

        currentSong.addEventListener("ended", () => document.getElementById("next")?.click());
